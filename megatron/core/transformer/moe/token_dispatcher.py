...
class MoEFlexTokenDispatcher(MoETokenDispatcher):
    ...
    def __init__(
        ...
        self._comm_manager: Optional[_DispatchManager] = None
        ...
        if backend != "deepep" and not self.config.moe_enable_deepep:
            raise ValueError(
        ...
        assert (
            self.config.moe_pad_expert_input_to_capacity is False
        ), "Flex token dispatcher with deepep backend does not support --moe-pad-expert-input-to-capacity"
        ...
        if _use_network_engine_for_baseline():
            try:
                resolved_backend, _ = get_global_network_engine().resolve(
                    ParallelDomain.EP,
                    intranode=group_intranode,
                )
                use_deepep = resolved_backend.name == "deepep"
            except Exception:
                pass
        ...
        if use_deepep:
            ...
            if not self._deepep_preflight_ok():
                logger.warning(
                    "[DeepEP][flex-init] preflight failed, fallback to torch alltoall dispatcher; tp_ep_group_ranks=%s",
                    group_ranks,
                )
            ...
            logger.warning(
                "[DeepEP][flex-init] dispatcher=torch_alltoall tp_ep_group_ranks=%s intranode=%s",
                group_ranks,
                group_intranode,
            )
            self._switch_to_torch_fallback(
    ...
    def _switch_to_torch_fallback(
        ...
        *,
        ...
        self.cudagraph_attrs = list(getattr(self._fallback_dispatcher, 'cudagraph_attrs', []))

    def _deepep_preflight_ok(self) -> bool:
        """Probe DeepEP buffer init once and synchronize the decision in current TPxEP group."""
        ok = 1
        try:
            ...
            ok = 0
            logger.warning("[DeepEP][preflight] local_failure err=%s", exc)
        ...
        if torch.distributed.is_initialized() and self.tp_ep_group is not None:
            ...
            torch.distributed.all_reduce(
                flag, op=torch.distributed.ReduceOp.MIN, group=self.tp_ep_group
            )
            ok = int(flag.item())
        ...
        *,
        ...
        if force_sync:
            ...
            if not _DEEPEP_RUNTIME_OVERRIDE_LOGGED:
                logger.warning(
                    "[DeepEP] DEEPEP_FORCE_SYNC=1: overriding %s to async_finish=False, allocate_on_comm_stream=False",
                    op_name,
                )
